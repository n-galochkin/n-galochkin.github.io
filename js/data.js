const PLAYER = {
  name:     'Nikita Galochkin',
  handle:   'Player <span>Profile</span>',
  title:    'Senior Unreal Engine programmer',
  server:   'Da Nang · Remote',
  status:   'Available for Contract',
  stats: [
    { label: 'Professional Experience',        value: '10+ years', color: 'orange'},
    { label: 'AAA Experience',          value: '5 years' },
    { label: 'Titles Shipped',          value: '6' },
    { label: 'Engines',                 value: 'Unreal Engine, Unity'},
      { label: 'Primary Languages',       value: 'C++, C#, Python'}, 
      { label: 'Languages',                 value: 'English B1,\n Russian Native' },
    { label: 'Timezone',                value: 'Asia' },
  ],
};  

const CAREER_OVERVIEW = {
  bio: 'Unreal Engine developer with 10+ years of experience in gameplay, network and AI programming for multiplayer PC, mobile\n' +
      'and VR games in various genres, including shooters, melee fighters, and open-world MMO RPGs. Also has skills in leading a\n' +
      'development team, developing CI/CD pipelines, and optimizing performance. Has worked on published AAA titles in\n' +
      'PC/Consoles. Master’s degree in software engineering',
};

const SKILLS = [
  { label: 'GAS',                tier: 1 },
  { label: 'CommonUI · MVVM · UMG' },
  { label: 'Enhanced Input' },
  { label: 'Gameplay Framework' },
  { label: 'Network Replication' },
  { label: 'Game Features' },
  { label: 'Team management' },
  { label: 'Metahuman · Mutable Customization' },
  { label: 'k8s · Agones · Docker · Dedicated server' },
  { label: 'HTN · Behavior tree · EQS · Navigation' },
  { label: 'EOS · Easy Anti-Cheat' },
  { label: 'WebSocket · Quic Protocol' },
  { label: 'Geometry Scripting' },
  { label: 'GRPC · Protobuf' },
  { label: 'CI/CD · Sentry' },
  { label: 'Gauntlet · Unit Testing · Automation Spec' },
  { label: 'Graphana' },
  { label: 'Perforce / Git' },
  { label: 'Unreal Insights' },
  { label: 'Matchmaking' },
  { label: 'Editor Tools' },
];

const PROJECTS = [
  {
    id:     'atone',
    name:   'Atone',
    tier:   'legendary',
    badge:  'aaa',
    status: { type: 'hold', label: '✕ Canceled' },
    date:   'Dec 2025 – 2026',
    role:   'Sr. Client Programmer',
    studio: 'GS Studio · Cyprus',
    desc:   'Large-scale action-adventure MMORPG built entirely in-house over 5 years. Desert/Middle-Eastern open world in UE5 with custom dedicated servers (70 CCU per layer), server-authoritative netcode, IRIS replication and Nanite geometry.',
    tags:   ['UE5','Action', 'MMORPG', 'Open world', 'Third Person'],
    links:  [
      { label: 'Website', url: 'https://www.gs-studio.eu/work/atone' },
    ],
    gallery: [
      { type: 'youtube', videoId: 'ct3dSTfC3TE', alt: 'Atone gameplay video' },
      { type: 'image', src: 'media/atone/1.jpg', alt: 'Atone screenshot 1' },
      { type: 'image', src: 'media/atone/2.jpg', alt: 'Atone screenshot 2' },
      { type: 'image', src: 'media/atone/3.jpg', alt: 'Atone screenshot 3' },
      { type: 'image', src: 'media/atone/4.jpg', alt: 'Atone screenshot 4' },
      { type: 'image', src: 'media/atone/5.jpg', alt: 'Atone screenshot 5' },
    ],
  },
  {
    id:     'majestic',
    name:   'GTA Roleplay',
    tier:   'legendary',
    badge:  'aaa',
    status: { type: 'hold', label: '? NDA' },
    date:   '2025',
    role:   'Sr. Gameplay Programmer',
    studio: 'Majestic Games · London',
    desc:   'Some NDA project by studio who made GTA-style open-world roleplay with deep progression and 500k+ active players. London studio title with a large existing codebase.',
    tags:   ['UE5', 'Mobile', 'NDA'],
    links:  [],
    gallery: [
      { type: 'youtube', videoId: '6dOGwQjXWNw', alt: 'Majestic Role Play trailer' },
      { type: 'image',   src: 'media/majestic/1.png', alt: 'Majestic Roleplay screenshot 1' },
      { type: 'image',   src: 'media/majestic/2.png', alt: 'Majestic Roleplay screenshot 2' },
      { type: 'image',   src: 'media/majestic/3.png', alt: 'Majestic Roleplay screenshot 3' },
      { type: 'image',   src: 'media/majestic/4.png', alt: 'Majestic Roleplay screenshot 4' },
      { type: 'image',   src: 'media/majestic/5.jpg', alt: 'Majestic Roleplay screenshot 5' },
      { type: 'image',   src: 'media/majestic/6.png', alt: 'Majestic Roleplay screenshot 6' },
    ],
  },
  {
    id:     'boss-fighters',
    name:   'Boss Fighters',
    tier:   'rare',
    badge:  'aa',
    status: { type: 'win', label: '✓ Shipped' },
    date:   '2023 – 2025',
    role:   'Lead Programmer',
    studio: 'Pixward Games · Estonia',
    desc:   'Asymmetrical multiplayer — VR player becomes the massive Boss, manipulating a physics sandbox arena. PC team plays as Fighters using strategy and environment to take it down.',
    tags:   ['UE5', 'VR/PC', 'FPS', 'Shooter', 'Multiplayer', 'Meta quest', "Game-Fi", "F2P"],
    links:  [
      { label: 'Steam',   url: 'https://store.steampowered.com/app/1784650/BOSS_FIGHTERS/' },
      { label: 'Website', url: 'https://www.bossfighters.game/' },
    ],
    gallery: [
      { type: 'youtube', videoId: 'TQRobo2zft8', alt: 'Boss Fighters trailer' },
      { type: 'image',   src: 'media/boss-fighters/1.jpg', alt: 'Boss Fighters screenshot 1' },
      { type: 'image',   src: 'media/boss-fighters/2.jpg', alt: 'Boss Fighters screenshot 2' },
      { type: 'image',   src: 'media/boss-fighters/3.jpg', alt: 'Boss Fighters screenshot 3' },
    ],
  },
  {
    id:     'mygames',
    name:   'Open World MMORPG',
    tier:   'legendary',
    badge:  'aaa',
    status: { type: 'hold', label: '✕ Canceled' },
    date:   '2022 – 2023',
    role:   'Sr. AI Programmer',
    studio: 'MY.GAMES · Global · 100+ team',
    desc:   'Massive open-world sandbox MMORPG under MY.GAMES publishing inspired by Genshin and Zelda.',
    tags:   ['UE4', 'Open World', 'Sandbox', "Multiplayer"],
    links:  [],
    gallery: [
      { type: 'image', src: 'media/mygames/1.jpg', alt: 'Project screenshot 1' },
      { type: 'image', src: 'media/mygames/2.jpg', alt: 'Project screenshot 2' },
    ],
  },
  {
    id:     'blood-of-heroes',
    name:   'Blood of Heroes',
    tier:   'legendary',
    badge:  'aaa',
    status: { type: 'win', label: '✓ Shipped' },
    date:   '2019 – 2022',
    role:   'Software Engineer',
    studio: 'Vizor Games · Minsk · 100+ team',
    desc:   'Dark fantasy PvP melee arena fighter inspired by For Honor. Warriors battle in medieval Nordic arenas with directional combat and stamina mechanics. Released on PC — PS4/PS5 & Xbox in progress before cancellation.',
    tags:   ['UE4', 'Action', 'Melee fighting', 'PvP Arena', 'Third Person'],
    links:  [{ label: 'Trailer', url: 'https://www.youtube.com/watch?v=k1dJ1VYJVNg' }],
    gallery: [
      { type: 'youtube', videoId: 'k1dJ1VYJVNg', alt: 'Blood of Heroes trailer' },
      { type: 'image',   src: 'media/blood-of-heroes/1.jpg', alt: 'Blood of Heroes screenshot 1' },
      { type: 'image',   src: 'media/blood-of-heroes/2.jpg', alt: 'Blood of Heroes screenshot 2' },
    ],
  },
  {
    id:     'loco-parentis',
    name:   'Loco Parentis',
    tier:   'rare',
    badge:  'aa',
    status: { type: 'win', label: '✓ Shipped' },
    date:   '2017 – 2019',
    role:   'Programmer',
    studio: 'Fair Games Studio · Kazan',
    desc:   'Surreal co-op VR+PC horror — help a girl find her mother in an endless, monster-filled building. Supports VR+VR, VR+PC, and PC+PC co-op combinations.',
    tags:   ['UE4', 'PC/VR', 'Co-op', 'Horror'],
    links:  [{ label: 'Steam', url: 'https://store.steampowered.com/app/693800/Loco_Parentis/' }],
    gallery: [
      { type: 'youtube', videoId: 'YHb0HB64ZqQ', alt: 'Loco Parentis trailer' },
      { type: 'image',   src: 'media/loco-parentis/1.jpg', alt: 'Loco Parentis screenshot 1' },
      { type: 'image',   src: 'media/loco-parentis/2.jpg', alt: 'Loco Parentis screenshot 2' },
      { type: 'image',   src: 'media/loco-parentis/3.jpg', alt: 'Loco Parentis screenshot 3' },
    ],
  },
  {
    id:     'protocol',
    name:   'Protocol',
    tier:   'rare',
    badge:  'aa',
    status: { type: 'win', label: '✓ Shipped' },
    date:   '2016 – 2018',
    role:   'Programmer',
    studio: 'Fair Games Studio · Kazan',
    desc:   'Sci-fi comedy adventure with dark humour. Carry out the Protocol — a strict set of government rules for making first contact with an alien life form shot down in the Arctic Circle.',
    tags:   ['UE4', 'PC/VR', 'Sci-Fi', 'Adventure' ],
    links:  [{ label: 'Steam', url: 'https://store.steampowered.com/app/724490/Protocol/' }],
    gallery: [
      { type: 'youtube', videoId: 'kJdMoBnWK5o', alt: 'Protocol trailer' },
      { type: 'image',   src: 'media/protocol/1.jpg', alt: 'Protocol screenshot 1' },
      { type: 'image',   src: 'media/protocol/2.jpg', alt: 'Protocol screenshot 2' },
      { type: 'image',   src: 'media/protocol/3.jpg', alt: 'Protocol screenshot 3' },
    ],
  },
  {
    id:     'mages-mazes',
    name:   'Mages Mazes',
    tier:   'uncommon',
    badge:  'a',
    status: { type: 'win', label: '✓ Shipped' },
    date:   '2015 – 2017',
    role:   'Programmer',
    studio: 'GD Forge · Kazan',
    desc:   'Mobile online rogue-like — fight monsters and other players in a magical, constantly changing dungeon. Find keys, unlock chests, collect rewards in an ever-shifting maze.',
    tags:   ['Unity', 'Mobile', 'Rogue-like', 'Online Multiplayer'],
    links:  [],
    gallery: [
      { type: 'youtube', videoId: 'BUKej18sdvI', alt: 'Mages Mazes gameplay' },
    ],
  },
  {
    id:     '36-strategists',
    name:   '36 Strategists',
    tier:   'uncommon',
    badge:  'a',
    status: { type: 'win', label: '✓ Shipped' },
    date:   '2015 – 2016',
    role:   'Programmer',
    studio: 'GD Forge · Kazan',
    desc:   'Mobile adaptation of a classic Chinese strategy board game — turn-based tactics rooted in ancient Chinese military philosophy. 80+ languages, online multiplayer.',
    tags:   ['Unity', 'Mobile', 'Turn-Based', 'Strategy', 'Online Multiplayer'],
    links:  [],
    gallery: [
      { type: 'youtube', videoId: 'QyZCI1gq7Us', alt: '36 Strategists gameplay' },
    ],
  },
];

const SEASONS = [
  {
    season:  'Season 11',
    period:  'Nov 2025 – Now',
    role:    'Senior Client Programmer',
    company: 'GS Studio · Remote',
    points:  [
      'Integrated EOS for online sessions',
      'Refactored client network layer using Quic protocol',
      'Investigated and decomposed automated test bot',
    ],
  },
  {
    season:  'Season 10',
    period:  'Jan 2025 – Nov 2025',
    role:    'Senior Gameplay Programmer',
    company: 'Majestic Games · Remote',
    points:  [
      'Developed core gameplay features',
      'Built Unreal Editor tools for exporting gameplay data to backend',
      'Redesigned UI extension system and layers using CommonUI',
      'Refactored interaction system',
    ],
  },
  {
    season:  'Season 8–9',
    period:  'Jun 2023 – Jan 2025',
    role:    'Lead Unreal Engine Programmer',
    company: 'Pixward Games · Remote',
    points:  [
      'Rearchitected network from P2P to client-server with reworked gameplay systems',
      'Developed services communication system based on WebSocket',
      'Built UI features using Viewmodel and Game Features plugins',
      'Integrated CI/CD, Sentry, Epic Online Services and Easy Anti-Cheat',
      'Led a growing team of programmers',
    ],
  },
  {
    season:  'Season 7',
    period:  'Jun 2022 – Jun 2023',
    role:    'Senior AI Programmer',
    company: 'MY.GAMES · Remote',
    points:  [
      'Improved AI behavior and character systems',
      'Created new nodes for HTN planning system',
      'Adapted AI navigation with climbing and jumping capabilities',
      'Developed nav-links generator for large open world — saved level-designer hours per sprint',
    ],
  },
  {
    season:  'Season 4–6',
    period:  'Jan 2019 – Jun 2022',
    role:    'Software Engineer',
    company: 'Vizor Games · Minsk, Belarus',
    points:  [
      'Developed core gameplay and metagame features',
      'Integrated GAS and implemented various combat abilities',
      'Optimized gameplay performance via Unreal Insights — CPU time reduced by 27%',
      'Enhanced testing with Automation Spec (unit tests) and Gauntlet (automation)',
    ],
  },
  {
    season:  'Season 1–3',
    period:  'Nov 2015 – Dec 2018',
    role:    'Game Developer',
    company: 'Fair Games Studio · Kazan, Russia',
    points:  [
      'Fullstack development for two mobile games (Unity / C#)',
      'AI, gameplay & VFX for VR cooperative games on Unreal Engine 4',
    ],
  },
];

const MEDALS = [
  { tier: 't3', icon: '⎈',  name: 'Architector',   desc: 'Led full P2P → Client-Server migration on a live VR-PC shooter.' },
  { tier: 't3', icon: 'AAA', name: 'Triple-A Shipped',    desc: 'Contributed to AAA published titles on PC &amp; Console.' },
  { tier: 't3', icon: 'X',   name: 'Decade Veteran',      desc: '10+ years of professional Unreal Engine development.' },
  { tier: 't2', icon: '★',   name: 'Lead Programmer',     desc: 'Led and grew a programmer team at Pixward Games.' },
  { tier: 't2', icon: '↑',  name: 'Open Source Hero',       desc: 'Pushed accepted important fixes to HTN open-source plugin.' },
  { tier: 't1', icon: '∞',   name: 'CI/CD Engineer',      desc: 'Built pipelines from scratch with Sentry monitoring &amp; EAC.' },
  { tier: 'locked', icon: '?', name: 'Open Source Hero',  desc: 'Publish a public UE plugin or dev tool. <em>In Progress.</em>' },
  { tier: 'locked', icon: '?', name: 'GDC Speaker',       desc: 'Talk at a major industry conference. <em>Career Goal.</em>' },
];

const CONTACT = {
  email:        'work.n.galochkin@gmail.com',
  availability: 'Remote · Freelance · Full-time · Available Now',
  links: [
    { label: 'LinkedIn',   url: 'https://www.linkedin.com/in/n-galochkin' },
    { label: 'GitHub',     url: 'https://github.com/n-galochkin' },
    { label: 'Youtube',     url: 'https://www.youtube.com/channel/UCKqe2Z8Tcv4Jg1zpdk2O7mg' },
    { label: 'Instagram',     url: 'https://www.instagram.com/reborn_aimer/' },
  ],
};

const REPORTS = [
  {
    id:      '001',
    cat:     'tech',
    title:   'P2P → Client-Server: Migrating a Live Game\'s Netcode',
    excerpt: 'A practical breakdown of how I rearchitected Boss Fighters\' networking layer from peer-to-peer to authoritative client-server — the gotchas, the wins, and what I\'d do differently.',
    date:    'Coming Soon',
    url:     '#',
  },
  {
    id:      '002',
    cat:     'deepdive',
    title:   'HTN Planning in Open World MMORPG',
    excerpt: 'How we extended Hierarchical Task Networks for a Genshin-style sandbox: new node types, climbing/jumping AI, and a nav-links generator that saved level-designers hours per sprint.',
    date:    'Coming Soon',
    url:     '#',
  },
  {
    id:      '003',
    cat:     'tutorial',
    title:   'Cutting CPU by 27% with Unreal Insights',
    excerpt: 'A step-by-step walkthrough of profiling a shipped UE4 fighter — the tools, the bottlenecks I found, and the changes that delivered measurable, repeatable wins.',
    date:    'Coming Soon',
    url:     '#',
  },
];
